...
mod count_flag {
    ...
    fn test_count_flag_invalid_value() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-c", "invalid"]);
...
mod all_flag {
    ...
    fn test_all_flag_shows_all_commands() {
        ...
        let lines_all: Vec<&str> =
            stdout_all.lines().filter(|l| !l.is_empty()).collect();
        let lines_default: Vec<&str> =
            stdout_default.lines().filter(|l| !l.is_empty()).collect();
...
mod more_than_flag {
    ...
    fn test_more_than_flag_filters_commands() {
        ...
        let output_filtered =
            run_histop(&["-f", path.to_str().unwrap(), "-a", "-m", "2"]);
        ...
        let lines_filtered: Vec<&str> =
            stdout_filtered.lines().filter(|l| !l.is_empty()).collect();
    ...
    fn test_more_than_flag_with_zero() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-m", "0", "-c", "1"]);
...
mod ignore_flag {
    ...
    fn test_ignore_single_command() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-a", "-i", "ls"]);
        let stdout = String::from_utf8_lossy(&output.stdout);
...
mod bar_size_flag {
    ...
    fn test_bar_size_flag() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-b", "10", "-c", "1"]);
    ...
    fn test_bar_size_invalid() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-b", "invalid"]);
...
mod no_bar_flag {
    ...
    fn test_no_bar_flag_hides_bar() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-n", "-c", "3"]);
        let stdout = String::from_utf8_lossy(&output.stdout);
...
mod no_percentage_flags {
    ...
    fn test_no_percentage_flag() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-np", "-c", "3"]);
    ...
    fn test_no_cumulative_flag() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-nc", "-c", "3"]);
...
mod output_format_flag {
    ...
    fn test_output_csv() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-o", "csv", "-c", "3"]);
        let stdout = String::from_utf8_lossy(&output.stdout);
    ...
    fn test_output_invalid_format() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "-o", "invalid"]);
...
mod color_flag {
    ...
    fn test_color_invalid() {
        let path = fixtures_path().join("bash_history");
        let output =
            run_histop(&["-f", path.to_str().unwrap(), "--color", "invalid"]);
...
mod config_flag {
    ...
    fn test_config_flag_with_valid_config() {
        ...
        assert!(
            lines.len() <= 2,
            "Expected at most 2 lines from config, got {}",
            lines.len()
        );
    ...
    fn test_cli_precedence_over_config_is_order_independent() {
        ...
        let config_path =
            unique_temp_path("histop_test_config_precedence", ".toml");
        ...
        let lines1: Vec<&str> =
            stdout1.lines().filter(|l| !l.is_empty()).collect();
        let lines2: Vec<&str> =
            stdout2.lines().filter(|l| !l.is_empty()).collect();
    ...
    fn test_config_flag_with_invalid_color_value() {
        let path = fixtures_path().join("bash_history");
        let config_path =
            unique_temp_path("histop_test_config_invalid_color", ".toml");
